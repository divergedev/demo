...
# 4. Creates an HTTPRoute that routes x-preview-id:<id> to the preview pod
#
...
TMPDIR=$(mktemp -d)
cp -r "${SERVICES_DIR}/demo-payments-api/"* "$TMPDIR/" 2>/dev/null || \
    { echo "Service not found. Run ./scripts/setup.sh first."; exit 1; }

# Patch version to show this is a preview
...
echo -e "    ./scripts/cleanup-preview.sh ${PREVIEW_ID}"
echo ""
